  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50">
          ...
          <div className="flex justify-between h-16 lg:h-20">
            <div className="flex items-center">
              <Link href="/" className="group flex items-center" onClick={() => setMenuOpen(false)}>
                <span className="gold-text text-lg sm:text-xl lg:text-2xl font-bold tracking-tight transition-opacity duration-300 group-hover:opacity-80">
                  Баланс и право
                </span>
              </Link>
            </div>
              ...
              {links.map((link) => {
                ...
                return (
                  <Link
                    key={link.href}
                    href={link.href}
                    className={`relative flex h-full items-center px-3 lg:px-5 text-[17px] font-medium tracking-wide transition-colors duration-300
                      after:absolute after:inset-x-3 lg:after:inset-x-4 after:bottom-0 after:h-[2px] after:origin-left after:rounded-full
      ...
      <div className={`md:hidden ${menuOpen ? 'block' : 'hidden'}`} id="mobile-menu">
        <div className="border-t border-[#D5A329]/10 bg-[#120907]/95 backdrop-blur-2xl px-4 pt-3 pb-5 space-y-1 shadow-[0_24px_48px_-16px_rgba(0,0,0,0.7)]">
          {links.map((link) => {
            const isActive = pathname === link.href;
            return (
              <Link
